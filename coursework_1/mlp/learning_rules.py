...
class GradientDescentLearningRule(object):
    ...
    def update_params(self, grads_wrt_params, sheduled_learning_rate=None):
        ...
        """
        # if sheduled_learning_rate= None use given learning rate (static), else use sheduler defined in optimizer.py
        if (sheduled_learning_rate==None):
            sheduled_learning_rate = self.learning_rate
       # print ('Learing_rate : ' + str(sheduled_learning_rate))
    
        for param, grad in zip(self.params, grads_wrt_params):
            param -= sheduled_learning_rate * grad
...
class AdamLearningRule(GradientDescentLearningRule):
    ...
    def update_params(self, grads_wrt_params, sheduled_learning_rate=None):
        ...
        for param, mom_1, mom_2, grad in zip(self.params, self.moms_1, self.moms_2, grads_wrt_params):
            ...
            mom_1_hat =(mom_1)/(1. - self.beta_1**t)
            mom_2 *= self.beta_2 
            mom_2 += (1. - self.beta_2) * (grad**2)
            mom_2_hat = (mom_2)/(1. - self.beta_2**t)    
            param -= (sheduled_learning_rate * mom_1_hat) / ((mom_2_hat + self.epsilon)**0.5)
...
class AdamLearningRuleWithWeightDecay(GradientDescentLearningRule):
    ...
    def update_params(self, grads_wrt_params, sheduled_learning_rate=None):
        ...
        for param, mom_1, mom_2, grad in zip(self.params, self.moms_1, self.moms_2, grads_wrt_params):
            ...
            # Tip
            h_t = sheduled_learning_rate/self.initial_learning_rate

            param_update = h_t*(((self.initial_learning_rate* mom_1_hat) / ((mom_2_hat + self.epsilon)**0.5)+self.weight_decay*param))  
            param -= param_update
...
class RMSPropLearningRule(GradientDescentLearningRule):
    ...
    def reset(self):
        ...
        second moment estimates.
        """
        for rms_grad in self.rms_grads:
            rms_grad *= 0.
    ...
    def update_params(self, grads_wrt_params, sheduled_learning_rate=None):
        ...
        Args:
            grads_wrt_params: A list of gradients of the scalar loss function
                ...
                m_2 = beta * m_2_prev + (1 - beta) * g**2
        """
        for param, rms_grad, grad in zip(self.params, self.rms_grads, grads_wrt_params):
            rms_grad *= self.beta 
            rms_grad += (1-self.beta)*(grad**2)
            param -= (self.learning_rate * grad) /((rms_grad + self.epsilon)**0.5)