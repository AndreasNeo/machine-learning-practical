...
class Optimiser(object):
    ...
    def __init__(self, model, error, learning_rule, train_dataset,
        ...
        # No restarts: total_iters_per_period=toal mumber of epochs set
        self.cosine_scheduler = CosineAnnealingWithWarmRestarts(min_learning_rate=0.0001, max_learning_rate=0.001,
                                                       total_iters_per_period=50.,
                                                       max_learning_rate_discount_factor=0.9,
                                                       period_iteration_expansion_factor=1)
    def do_training_epoch(self, updated_learning_rate):
        ...
        """
        print ('Learning rate:' + str(updated_learning_rate))
        with self.tqdm_progress(total=self.train_dataset.num_batches) as train_progress_bar:
            ...
            for inputs_batch, targets_batch in self.train_dataset:
                ...
                # sheduled_learning_rate=None -> Don't use sheduler. 
                # sheduled_learning_rate=updated_learning_rate -> Use sheduler
                self.learning_rule.update_params(grads_wrt_params,sheduled_learning_rate=updated_learning_rate) 
    ...
    def train(self, num_epochs, stats_interval=5):
        ...
        with self.tqdm_progress(total=num_epochs) as progress_bar:
            ...
            for idx, epoch in enumerate(range(1, num_epochs + 1)):
                start_time = time.time()
                updated_learning_rate = self.cosine_scheduler.update_learning_rule(learning_rule=self.learning_rule, epoch_number=idx)
                print (idx)
                #self.sheduler
                self.do_training_epoch(updated_learning_rate)
